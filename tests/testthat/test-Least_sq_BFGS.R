test_that("BFGS vs least_sq for linear model", {
  ...
  least_sq_out <- hiper_glm(design, outcome, model = 'linear', option = list(mle_solver = 'least_sq'))
  bfgs_out <- hiper_glm(design, outcome, model = 'linear', option = list(mle_solver = 'BFGS'))
